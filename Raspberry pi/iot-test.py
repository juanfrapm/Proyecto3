...
def publishData(txt):
    print(txt)
    last_processed_line = 0  # Para seguir el índice de la última línea procesada
    
    while True:
        time.sleep(5)
        with open("datos.csv", mode="r") as archivo:
            lineas = archivo.readlines()  # Leer todas las líneas
            if last_processed_line < len(lineas):  # Solo procesamos las nuevas líneas
                # Procesar las nuevas líneas
                for linea in lineas[last_processed_line + 1:]:  # Empezamos desde la última línea no procesada
                ...
                last_processed_line = len(lineas) - 1  # Actualizamos el índice de la última línea procesada
